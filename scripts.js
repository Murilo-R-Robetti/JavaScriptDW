...
for (let btn of btnBotoes) {
  btn.addEventListener("click", () => {
    adicionaNumero(calculadora, btn.textContent);
  });
}
...
for (let btn of btnOperacoes) {
  btn.addEventListener("click", () => {
    escolheOperador(calculadora, btn.textContent);
  });
}
/********
 * Regras da aplicação
...
function adicionaNumero(calculadora, numero) {
  // if (calculadora.operandoAtual.includes(".")) return;
 ...
 * Apaga o último dígito digitado na calculadora
 */
